...
export class PosterAuthService {
	...
	async getMerchantLoginAndCodeLink(placeId: string) {
		...
		const baseUrl = false && environment.production ? `https://dev-api.resty.od.ua` : `http://192.168.68.52:3000`;
	...
	async getAccessToken(body: PosterAccessCodeInput) {
		...
		const baseUrl = "http://192.168.68.52:3000/api/poster/auth-confirm";
